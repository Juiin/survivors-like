...
func open():
	...
	open_tween.tween_property(self, "global_position", global_position + Vector2(0, 100), 0.5).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)

func close():
	if (close_tween && close_tween.is_running()) || !visible:
		return
